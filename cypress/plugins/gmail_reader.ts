...
export class GmailReader {
   ...
   */
  createSearchQuery(from: string, to: string, subject: string) {
   ...
   */
  async getMessageId(gmail: any, searchQuery: string, searchInterval: number = 5000, tryLimit: number = 10) {
    try {
   ...
   */
  async getMessage(gmail: any, msgId: any) {
    try {
   ...
   */
  async readEmail(options: object = {}) {
    const auth = await this.authorize()
    const gmail = google.gmail({ auth, version: 'v1' })
    const queryString = this.createSearchQuery(options['from'], options['to'], options['subject'])
    const msgId = await this.getMessageId(gmail, queryString, options['searchInterval'], options['tryLimit'])
    const message = await this.getMessage(gmail, msgId)
    const readableMessage = this.decodeMessage(message)
