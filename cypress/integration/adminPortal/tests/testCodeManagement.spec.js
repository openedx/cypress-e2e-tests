...
describe('code management tests', function () {
  ...
  it('prepopulates email address and enterprise name fields', () => {
    ...
    codeManagementDashboard
      .getFormField('emailAddress')
      .should('have.attr', 'value', Cypress.env('ADMIN_USER_EMAIL'))
    codeManagementDashboard
      .getFormField('enterpriseName')
      .should('have.attr', 'value', Cypress.env('enterprise_name'))
  })
  ...
  it('Marks the correct fields as required REQUEST MORE CODES form', () => {
    ...
    codeManagementDashboard
      .getLabels('Company')
      .should('have.text', requestCodeslabelsAndText.companyLabel)
      .children()
      .should('have.class', 'required')
    codeManagementDashboard
      .getLabels('Email')
      .should('have.text', requestCodeslabelsAndText.emailLabel)
      .children()
      .should('have.class', 'required')
    codeManagementDashboard
      .getLabels('Codes')
      .should('have.text', requestCodeslabelsAndText.numberOfCodesLabel)
  })
  it('Has the correct buttons REQUEST MORE CODES form', () => {
    ...
    codeManagementDashboard
      ...
      .and('have.text', requestCodeslabelsAndText.requestCodesLabel)
    codeManagementDashboard
      .getCancelButton()
      .should(
        'have.attr',
        'href',
        `/${Cypress.env('enterprise_slug')}/admin/coupons`,
      )
  ...
  it('Verifies the validation checks on REQUEST MORE CODES form', () => {
    ...
    codeManagementDashboard
      .getInvalidFeedback()
      .should('have.text', requestCodeslabelsAndText.fieldRequiredError)
    ...
    codeManagementDashboard
      .getInvalidFeedback()
      .should('have.text', requestCodeslabelsAndText.validEmailError)
    codeManagementDashboard
      .getRequestCodesButton()
  ...
  it('Verifies the correct functioning of REQUEST CODES form', () => {
    ...
    cy.url().should(
      'eq',
      `${Cypress.config().baseUrl}/${Cypress.env('enterprise_slug')}${urls.requestCodes}`,
    )
    codeManagementDashboard.getCancelButton().click()
    cy.url().should(
      'eq',
      `${Cypress.config().baseUrl}/${Cypress.env('enterprise_slug')}${
        urls.codeManagement
      }`,
    )
    codeManagementDashboard.requestMoreCodes()
  ...
  describe('checks filter options and coupon metadata', () => {
    beforeEach(() => {
      cy.get('@couponId').then((couponId) => {
        coupons.fetchCouponReport(couponId).then((response) => {
          ...
          cy.wrap(couponReport.match(/\w+ \d+, \d+/g)).as('dates')
    ...
    it('verifies the metadata on the coupon table header', () => {
      codeManagementDashboard.getCouponMeta().then((couponInfo) => {
        ...
        cy.get('@dates').then((dates) => {
          const couponMeta = {
            validCouponReportFromDate:
              HelperFunctions.convertDateToShortFormat(dates[1]),
            validCouponTableFromDate:
              HelperFunctions
                .convertDateToShortFormat(couponInfo.eq(1).text()),
            validCouponReportToDate:
              HelperFunctions.convertDateToShortFormat(dates[2]),
            validCouponTableToDate:
              HelperFunctions
                .convertDateToShortFormat(couponInfo.eq(2).text()),
            remainingAssignments: couponInfo.eq(3).text(),
            enrollmentsRedeemed: couponInfo.eq(4).text(),
          }
          expect(couponMeta.validCouponReportFromDate).to.eql(couponMeta.validCouponTableFromDate)
          expect(couponMeta.validCouponReportToDate).to.eql(couponMeta.validCouponTableToDate)
          cy.get('@quantity').then((quantity) => {
            expect(quantity).to.eql(couponMeta.remainingAssignments)
          })
  ...
  it.only('checks for the assignment and revoking of the coupons', () => {
    ...
    cy.get('@couponId').then((couponId) => {
      cy.route('GET', `**/${couponId}/codes/?code_filter=unassigned**`).as('results')
      coupons.fetchCouponReport(couponId).then((response) => {
        const couponReport = response.body
        const [couponName] = couponReport.match(/Test_Coupon_\w+/g)
        codeManagementDashboard.getCoupon().contains(couponName).click()
    ...
    cy.wait('@results').then((xhr) => {
      ...
      codeManagementDashboard
        ...
        .should('have.text', remainingRedemptions.toString())
    })
    ...
    codeManagementDashboard
      ...
      .should('have.text', 'Successfully revoked code(s)')
  })
